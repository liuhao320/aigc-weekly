...
**必须抓取 REFERENCE.md 中列出的所有信息源（共 25 个独立源）**：

- Important Resources: 10 个
- Blogs & Websites: 13 个
- KOL & Influencers: 3 个

**注意**：Hacker News front 需要按 start_date 到 end_date 之间的每一天展开成多个 URL，因此实际抓取任务数 > 25。
...
2.  **分析与规划 (Analyze & Plan)**：
    - **关键**：完整读取 `REFERENCE.md` 文件，获取**所有 25 个独立信息源**
    - **⚠️ 强制要求：必须抓取所有 25 个独立源，不得跳过任何源**
    - **必须处理以下三个分类**：
    ...
    - **Hacker News 处理**：为 `start_date` 到 `end_date` 之间的每一天生成 URL（例如 7 天周期生成 7 个 URL）
    - **验证**：确认从 REFERENCE.md 提取的独立信息源数量**必须等于 25 个**，否则重新读取
    - **列出所有源**：在开始抓取前，在消息中明确列出将要抓取的所有源（25 个独立源，HN 按日期展开后会产生更多任务）
...
4.  **结果验证与汇总**：
    ...
    - **严格验证**：确认已尝试抓取所有 25 个独立信息源（包括成功和失败）
    - **⚠️ 强制要求：如果尝试抓取的独立源少于 25 个，任务视为失败**
    - 生成抓取报告，**必须包含所有 25 个独立源的状态**：
      - ✅ 成功抓取的源及文件数
      - ❌ 失败的源及原因
      - ⚠️ 如果某些源没有尝试抓取，在报告中标记为 "SKIPPED" 并说明原因
      - **报告必须逐一列出所有 25 个独立源的状态**
    - 将报告保存到 `logs/research-report.md`
    - **最终检查**：
      - 如果尝试抓取的独立源数量 < 25，在报告中添加 ❌ 错误标记
